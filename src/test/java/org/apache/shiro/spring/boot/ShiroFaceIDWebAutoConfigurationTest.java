...
class ShiroFaceIDWebAutoConfigurationTest {
    ...
    @Test
    @DisplayName("Auto-configuration loads when 'shiro.kisso.enabled=true'")
    void testLoadsWhenEnabledPropertySet() {
        runner.withUserConfiguration(ShiroFaceIDWebAutoConfiguration.class)
                .withPropertyValues("shiro.kisso.enabled=true")
                .run(context -> assertThat(context).hasSingleBean(ShiroFaceIDWebAutoConfiguration.class));
    }
    ...
    void testNotLoadedWhenPropertyAbsent() {
        runner.withUserConfiguration(ShiroFaceIDWebAutoConfiguration.class)
                .run(context -> assertThat(context).doesNotHaveBean(ShiroFaceIDWebAutoConfiguration.class));
    }
}
